...
def run(
    ...
    loss = torch.zeros(5, device=device)  # box, obj, cls, mmd, fg(与 ComputeLoss 5元组对齐)
    ...
    if mask:
        ...
        for batch_i, (im, targets, paths, shapes, ms) in enumerate(pbar):
            ...
            # Inference
            with dt[1]:
                # seg 模型 _forward_once 返回 (det_out, backbone_feature, mask_output)，普通模型返回 (det_out, backbone_feature)。
                # det_out 在 eval 下 = (inference_out, train_out)。统一取 det_out = out[0]：
                #   NMS(preds) 内部会取 preds[0]=inference_out；loss 用 preds[1]=train_out（见下）。
                out = model(im) if compute_loss else model(im, augment=augment)
                preds = out[0] if isinstance(out, (list, tuple)) else out

            # Loss
            if compute_loss:
                # loss += compute_loss(train_out, targets, feature_s=None, feature_t=None, mask=(mask_output ,ms))[1]  # box, obj, cls
                loss += compute_loss(preds[1], targets, feature_s=None, feature_t=None, mask=None)[1]  # box, obj, cls
                # loss += compute_loss(preds[1], targets, feature_s=None, feature_t=None, mask=(mask_output ,ms))[1]  # box, obj, cls
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            # Inference
            with dt[1]:
                p, _ = model(im) if compute_loss else (model(im, augment=augment), None)
                if len(p) == 2:
                    [preds, train_out] = p
                elif len(p) == 3:
                    [preds, train_out, ms] = p

            # Loss
            if compute_loss:
                loss += compute_loss(train_out, targets, feature_s=None, feature_t=None)[1]  # box, obj, cls
