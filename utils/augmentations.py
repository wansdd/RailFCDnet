...
def random_perspective(
    im, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0), mask=None
):
    ...
    return (im, targets, mask) if mask is not None else (im, targets)


def copy_paste(im, labels, segments, p=0.5, mask=None):
    """
    ...
    if p and n:
        ...
        # 如果mask不为空，创建对应的mask_new
        if mask is not None:
            mask_new = np.zeros(mask.shape, mask.dtype)
        ...
        for j in random.sample(range(n), k=round(p * n)):
            ...
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                ...
                # 如果mask存在，对mask_new进行相同的轮廓填充操作
                if mask is not None:
                    cv2.drawContours(mask_new, [segments[j].astype(np.int32)], -1, (1, 1, 1), cv2.FILLED)

        result = cv2.flip(im, 1)  # augment segments (flip left-right)
        i = cv2.flip(im_new, 1).astype(bool)
        im[i] = result[i]  # cv2.imwrite('debug.jpg', im)  # debug
        
        # 如果mask存在，对mask进行相同的翻转和替换操作
        if mask is not None:
            mask_result = cv2.flip(mask, 1)
            mask_i = cv2.flip(mask_new, 1).astype(bool)
            mask[mask_i] = mask_result[mask_i]
            return im, labels, segments, mask
            
    return (im, mask, labels, segments) if mask is not None else (im, labels, segments)
...
def mixup_mask(im, labels, mask, im2, labels2, mask2):
    """
    ...
    mask = (mask * 1 + mask2 * 1).astype(np.uint8)
    # mask = (mask * r + mask2 * (1 - r)).astype(np.uint8)
