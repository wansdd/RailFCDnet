...
class Loggers:
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi, domain=0):
        ...
        if self.csv:
            file = self.save_dir / f"results_{name}.csv"
        ...
        if self.tb:
            for k, v in x.items():
                if domain == 0:
                    self.tb.add_scalar(f"source/{k}", v, epoch)
                else:
                    self.tb.add_scalar(f"target/{k}", v, epoch)
