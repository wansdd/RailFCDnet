...
def img2label_paths(img_paths):
    ...
    extension with `.txt`. Falls back to a label next to the image for generated datasets.
    """
    sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}labels{os.sep}"  # /images/, /labels/ substrings
    label_paths = []
    for image_path in img_paths:
        standard_path = sb.join(image_path.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt"
        adjacent_path = image_path.rsplit(".", 1)[0] + ".txt"
        use_standard = os.path.isfile(standard_path) or not os.path.isfile(adjacent_path)
        label_paths.append(standard_path if use_standard else adjacent_path)
    return label_paths


def img2roi_paths(img_paths):
    """Generates ROI txt paths from image paths, preferring a sibling `/mask/` tree."""
    sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}mask{os.sep}"  # /images/, /mask/ substrings
    roi_paths = []
    for x in img_paths:
        if sa in x:
            roi_paths.append(sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt")
        else:
            p = Path(x)
            roi_paths.append(str(p.parent / "mask" / f"{p.stem}.txt"))
    return roi_paths
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.roi_warned = set()
        # self.mask_path = "/Programs/DA2/yolov5-master/result_00004594.png"
        # if self.mask:
        #     self.mask_img = cv2.imread(self.mask_path)
        #     assert self.mask_img is not None, f"Mask image not found at {self.mask_path}"
    ...
    def __getitem__(self, index):
        ...
        if mosaic := self.mosaic and random.random() < hyp["mosaic"]:
            ...
            if self.mask:
                img, labels, mask = self.load_mosaic(index,mask=self.mask)
            else:
                img, labels = self.load_mosaic(index,mask=self.mask)
            ...
            if random.random() < hyp["mixup"]:
                if self.mask:
                    img, labels, mask = mixup_mask(img, labels, mask, *self.load_mosaic(random.choice(self.indices), mask=self.mask))
                else:
            ...
            # Load image  在加载验证集
            if self.mask:
                img, (h0, w0), (h, w), mask = self.load_image(index,mask=self.mask)
            else:
            ...
            if self.roi_crop:
                if self.mask:
                    img, labels, (h, w), mask = self.apply_roi_crop(index, img, labels, (h0, w0), mask)
                else:
                    img, labels, (h, w) = self.apply_roi_crop(index, img, labels, (h0, w0))
            ...
            shapes = ((h, w), (ratio, pad)) if self.roi_crop else ((h0, w0), ((h / h0, w / w0), pad))
    ...
    def load_image(self, i,mask =False):
        """
        ...
        if mask:
            # New ablations keep masks in p_masks/o_masks beside the image directory.
            _mask_dirname = os.environ.get("RCD_MASK_DIRNAME", "")
            if _mask_dirname:
                mask_path_dir = os.path.join(os.path.dirname(os.path.dirname(f)), _mask_dirname, os.path.basename(f))
            else:
                _suf = os.environ.get("RCD_MASK_SUFFIX", "_mask")
                mask_path_dir = os.path.dirname(f) + _suf + '/' + f.split('/')[-1]
            mask_img = cv2.imread(mask_path_dir)
            assert mask_img is not None, f"Mask Image Not Found {mask_path_dir}"
            h0, w0 = mask_img.shape[:2]  # orig hw
            r = self.img_size / max(h0, w0)  # ratio
            if r != 1:  # if sizes are not equal
                interp = cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA
                mask_img = cv2.resize(mask_img, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)
        ...
        if im is None:  # not cached in RAM
            ...
            if mask:
                return im, (h0, w0), im.shape[:2], mask_img  # im, hw_original, hw_resized
            return im, (h0, w0), im.shape[:2]  # im, hw_original, hw_resized
        return self.ims[i], self.im_hw0[i], self.im_hw[i]  # im, hw_original, hw_resized

    def warn_roi_once(self, key, message):
        """Logs an ROI warning only once for the given key."""
        if key not in self.roi_warned:
            LOGGER.warning(message)
            self.roi_warned.add(key)

    def read_roi(self, i, original_hw, resized_hw):
        """Reads ROI from txt first, then falls back to a same-name mask image."""
        im_file = self.im_files[i]
        p = Path(im_file)

        roi_candidates = [Path(x) for x in img2roi_paths([im_file])]
        roi_candidates.extend(
            [
                Path(str(p.parent) + "_mask") / f"{p.stem}.txt",
                p.parent / "mask" / f"{p.stem}.txt",
            ]
        )
        if f"{os.sep}images{os.sep}" in im_file:
            roi_candidates.append(Path(im_file.replace(f"{os.sep}images{os.sep}", f"{os.sep}images_mask{os.sep}")).with_suffix('.txt'))

        roi_file = next((x for x in roi_candidates if x.exists()), None)
        if roi_file is not None:
            try:
                values = roi_file.read_text().strip().split()
                if len(values) != 4:
                ...
                gain_w = w / max(w0, 1)
                gain_h = h / max(h0, 1)
                roi = np.array([xmin * gain_w, ymin * gain_h, xmax * gain_w, ymax * gain_h], dtype=np.float32)
                roi[[0, 2]] = roi[[0, 2]].clip(0, w)
                roi[[1, 3]] = roi[[1, 3]].clip(0, h)
                ...
                self.warn_roi_once(
                    f"empty:{roi_file}",
                    f"WARNING ⚠️ ROI in '{roi_file}' is empty after clipping, falling back to ROI mask/original image.",
                )
            except Exception as e:
                self.warn_roi_once(
                    f"invalid:{roi_file}",
                    f"WARNING ⚠️ Failed to parse ROI file '{roi_file}': {e}. Falling back to ROI mask/original image.",
                )

        mask_candidates = [
            Path(str(p.parent) + "_mask") / p.name,
            p.parent / "mask" / p.name,
        ]
        if f"{os.sep}images{os.sep}" in im_file:
            mask_candidates.append(Path(im_file.replace(f"{os.sep}images{os.sep}", f"{os.sep}images_mask{os.sep}")))

        mask_file = next((x for x in mask_candidates if x.exists()), None)
        if mask_file is None:
            self.warn_roi_once(
                f"missing:{im_file}",
                f"WARNING ⚠️ ROI crop enabled but neither ROI txt nor ROI mask was found for '{im_file}', falling back to the original image.",
            )
            return None
        ...
        if mask is None:
            self.warn_roi_once(
                f"badmask:{mask_file}",
                f"WARNING ⚠️ Failed to read ROI mask '{mask_file}', falling back to the original image.",
            )
            return None
        ...
        if len(xs) == 0 or len(ys) == 0:
            self.warn_roi_once(
                f"emptymask:{mask_file}",
                f"WARNING ⚠️ ROI mask '{mask_file}' is empty, falling back to the original image.",
            )
            return None
    ...
    def apply_roi_crop(self, i, img, labels, original_hw, mask_img=None):
        """Crops one resized image to its ROI rectangle and remaps normalized labels into the cropped frame."""
        resized_h, resized_w = img.shape[:2]
        roi = self.read_roi(i, original_hw, (resized_h, resized_w))
        if roi is None:
            if mask_img is not None:
                return img, labels, img.shape[:2], mask_img
            return img, labels, img.shape[:2]
        ...
        if labels.size:
            boxes = xywhn2xyxy(labels[:, 1:].copy(), w=resized_w, h=resized_h)
            boxes[:, [0, 2]] = boxes[:, [0, 2]].clip(x1, x2)
            boxes[:, [1, 3]] = boxes[:, [1, 3]].clip(y1, y2)
            boxes[:, [0, 2]] -= x1
            boxes[:, [1, 3]] -= y1
            valid = (boxes[:, 2] > boxes[:, 0]) & (boxes[:, 3] > boxes[:, 1])
            labels = labels[valid]
            if labels.size:
                labels[:, 1:] = xyxy2xywhn(boxes[valid], w=img.shape[1], h=img.shape[0], clip=True, eps=1e-3)
    ...
    def load_mosaic(self, index ,mask=False):
        ...
        #     if r != 1:  # if sizes are not equal
        #         interp = cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA
        #         mask_img = cv2.resize(mask, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)

        for i, index in enumerate(indices):
            # Load image
            if mask:
                img, (h0, w0), (h, w),mask_img = self.load_image(index, mask=mask)
            else:   
                img, (h0, w0), (h, w) = self.load_image(index)
            ...
            if self.roi_crop:
                if mask:
                    img, labels, (h, w), mask_img = self.apply_roi_crop(index, img, labels, (h0, w0), mask_img)
                else:
                    img, labels, (h, w) = self.apply_roi_crop(index, img, labels, (h0, w0))
                segments = []
            ...
            if i == 0:  # top left
                ...
                if mask:
                    mask4 = np.full((s * 2, s * 2, mask_img.shape[2]), 0, dtype=np.uint8)  # base mask with 4 tiles
            ...
            if mask:
                mask4[y1a:y2a, x1a:x2a] = mask_img[y1b:y2b, x1b:x2b]  # mask4[ymin:ymax, xmin:xmax]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        # Augment
        if mask:
            img4, mask4, labels4, segments4 = copy_paste(img4, labels4, segments4, p=self.hyp["copy_paste"],mask=mask4)
            img4, labels4, mask4 = random_perspective(
                img4,
                labels4,
                segments4,
    ...
    def collate_fn(batch):
        ...
        if len(batch[0]) ==5:
            return torch.stack(im, 0), torch.cat(label, 0), path, shapes, torch.stack(mask, 0)
        else:
